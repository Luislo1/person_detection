...
ringbuf_t* rb_init(const char* name, uint32_t size) {
  ...
  // 1. Force allocation of the struct in PSRAM to save internal heap space
  r = (ringbuf_t*)heap_caps_malloc(sizeof(ringbuf_t), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  if (!r) {
      ESP_LOGE(RB_TAG, "Failed to allocate ringbuf_t struct");
      return NULL;
  }

  // 2. Force allocation of the buffer in PSRAM
  buf = (unsigned char*)heap_caps_calloc(1, size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  if (!buf) {
      ESP_LOGE(RB_TAG, "Failed to allocate ringbuf data of size %lu", size);
      free(r); // Clean up the struct if buffer fails
      return NULL;
  }
